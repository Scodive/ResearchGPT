...
export default function PaperGenerator() {
  ...
  const generatePaper = async () => {
    ...
    try {
      // 实际开发中这里会调用API
      // const response = await axios.post('/api/generate-paper', { topic: researchTopic });
      // setLatexContent(response.data.latex);
      // setPaperTitle(response.data.title);
      
      // 模拟API请求
      setTimeout(() => {
        const simulatedTitle = `Advancements in ${researchTopic}: A Comprehensive Review`;
        setPaperTitle(simulatedTitle);
        setLatexContent(generateSampleLatex(researchTopic, simulatedTitle));
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto py-8 px-4">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold mb-6 text-center">LaTeX 论文生成器</h2>
          <p className="text-lg text-gray-600 mb-8 text-center">
            输入研究主题，自动生成高质量的学术论文（IEEE格式）
          </p>
